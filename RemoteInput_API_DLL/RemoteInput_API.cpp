...
void RemoteInputAPI::execute(uint8_t * command) {
	...
	case INPUT_TYPE_MOUSE:
		...
		case MOUSE_ACTION_LEFT_CLICK:
			if (VERBOSITY) std::cout << "CLICKLCICKCLICK left click" << std::endl;
			MouseMovements(MOUSEEVENTF_LEFTDOWN, 0, 0, 0);
			MouseMovements(MOUSEEVENTF_LEFTUP, 0, 0, 0);
			break;
		case MOUSE_ACTION_RIGHT_CLICK:
			if (VERBOSITY) std::cout << "CLICKLCICKCLICK right click" << std::endl;
			MouseMovements(MOUSEEVENTF_RIGHTDOWN, 0, 0, 0);
			MouseMovements(MOUSEEVENTF_RIGHTUP, 0, 0, 0);
			break;
		case MOUSE_ACTION_LEFT_DOWN:
			if (VERBOSITY) std::cout << "CLICKLCICKCLICK left down" << std::endl;
			MouseMovements(MOUSEEVENTF_LEFTDOWN, 0, 0, 0);
			break;
		case MOUSE_ACTION_LEFT_UP:
			if (VERBOSITY) std::cout << "CLICKLCICKCLICK left up" << std::endl;
			MouseMovements(MOUSEEVENTF_LEFTUP, 0, 0, 0);
			break;
		case MOUSE_ACTION_MIDDLE_CLICK:
			if (VERBOSITY) std::cout << "CLICKLCICKCLICK middle click" << std::endl;
			MouseMovements(MOUSEEVENTF_MIDDLEDOWN, 0, 0, 0);
			MouseMovements(MOUSEEVENTF_MIDDLEUP, 0, 0, 0);
			break;
		case MOUSE_ACTION_MOVE:
			if (dataLength == 4) {
				if (lastXCoor - lastYCoor == 0) { //first time finger down // if both newXAverage and newYAverage are " -1 ", Doing newXAverage - newYAverage will be equal to 0
					...
					if (newXCoor - newYCoor == 0) { // if both newXCoor and newYCoor are " -1 ", Doing newXCoor - newYCoor will be equal to 0
						lastXCoor = -1;
						lastYCoor = -1;
					}
					else { //finger lifts, resetting all the condition
						if (VERBOSITY) printf("Received coordinate %d,%d \n", newXCoor - lastXCoor, newYCoor - lastYCoor);
		...
		case MOUSE_ACTION_SCROLL:
			if (dataLength == 4) {
				if (lastXAverage - lastYAverage == 0) { //first time finger down // if both newXAverage and newYAverage are " -1 ", Doing newXAverage - newYAverage will be equal to 0
					...
					if (newXAverage - newYAverage == 0) { // if both newXAverage and newYAverage are " -1 ", Doing newXAverage - newYAverage will be equal to 0
						lastXAverage = -1;
						lastYAverage = -1;
					}
					else { //finger lifts, resetting all the condition
						if (VERBOSITY) printf("Received Scroll %d,%d \n", newXAverage - lastXAverage, newYAverage - lastYAverage);
